...
export default function Slider({ options, setOption, selected }: SliderProps) {
  ...
  return (
    <View collapsable={false} className="flex flex-row mx-auto relative h-12 bg-rich_black rounded-xl py-2">
      <Animated.View
        className="-z-10 bg-midnight_green h-12 rounded-xl absolute left-0 "
        style={{ width: width, transform: [{ translateX: pos }] }}
      />
      <View className={"flex flex-row transition-all duration-500 " + (!selected && " divide-x-2 divide-powder_blue/40")} ref={buttonsRef} collapsable={false} >
      {options.map((v, i) => (
        <TouchableOpacity
          activeOpacity={1}
          key={i}
